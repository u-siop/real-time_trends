...
def extract_representative_info(trees, google_trends_keywords, used_rep_keywords, source='Naver'):
    trees_info = []
    for tree_info in trees:
        ...
        rep_keyword = select_representative_keyword(top_keywords, used_rep_keywords, google_trends_keywords)
        if not rep_keyword:
            continue  # 대표 키워드가 없으면 스킵
        # 중복 대표 키워드 방지
        used_rep_keywords.add(rep_keyword)
        # 대표 키워드 제외 상위 키워드
...
def main():
    ...
    for keyword, volume in sorted_keywords:
        ...
        if not keywords:
            continue
        # 기존 트렌드 트리들과 유사도 비교하여 병합
        merged = False
        for tree_info in trend_trees:
            similarity = calculate_jaccard_similarity(keywords, tree_info['all_keywords'])
            if similarity >= 0.17:  # 유사도 임계값을 낮춤
                tree_info['articles'].append({
                    'title': keyword,
                ...
                tree_info['importance'] += volume  # 검색량 누적
                tree_info['contains_trend'] = True
                merged = True
                logging.info(f"Google 트렌드 트리 병합 완료: {keyword} (유사도: {similarity:.2f})")
                break
        if not merged:
            # 새로운 트렌드 트리 생성
            trend_trees.append({
    ...
    used_rep_keywords = set()  # 중복 대표 키워드 방지를 위한 집합
    ...
    naver_trees_info = extract_representative_info(naver_trees, trending_keywords, used_rep_keywords, source='Naver')
    trend_trees_info = extract_representative_info(trend_trees, trending_keywords, used_rep_keywords, source='Google Trends')
    ...
    for item in top_naver_issues:
        phrase = item['phrase']
        if phrase not in seen_phrases:
            final_issues.append(item)
            seen_phrases.add(phrase)
        if len(final_issues) >= 6:
            break

    for item in top_trend_issues:
        phrase = item['phrase']
        if phrase not in seen_phrases:
            final_issues.append(item)
            seen_phrases.add(phrase)
